...
class Home extends React.Component {
  ...
  searchHandle = () => {
    ...
    search = (event, data) =>{
        let value = data.value != null ? data.value : this.state.searchKeyWord;
        let page = data.page ? data.page : 1;
        let _this =this;
        _this.searchHandle();
        this.setState({searchKeyWord:value});
        axios.get('http://127.0.0.1:18080/search?keyWord='+value+'&page=' + page).then((data)=>{
        _this.setState({searchData:data.list,totalPage:data.totalPage, hotWord:data.hotWord});
    });
}
  ...
  render() {
    ...
    return (
      <div className='home-container'>
        ...
        {this.state.searchBarFlag?<SearchBar hotWord={this.state.hotWord} searchPage={this.search} totalPage={this.state.totalPage} searchData={this.state.searchData} hideSearchBar={this.hideSearchBar}/>:null}
        <Dimmer inverted active={this.state.globalLoading} page>
          <Loader>Loading</Loader>
        </Dimmer>
        <div className="home-topbar">
            {/*onBlur={this.hideSearchBar} onFocus={this.searchHandle}    value={this.state.searchKeyWord}   */}
          <Input onChange={this.search.bind(this) }  value={this.state.searchKeyWord} fluid icon={{ name: 'search', circular: true, link: true }} placeholder='搜房源...' />
        </div>
